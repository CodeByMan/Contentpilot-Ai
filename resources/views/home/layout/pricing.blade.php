...
<div class="container">
 ...
 @php
    ...
    $headingTitle = $heading?->title ?? 'Simple pricing for creators and teams';
    $headingDescription = $heading?->description ?? 'Start free, then upgrade when you need more templates, higher word limits, and advanced AI tools.';
...
@endphp
   ...
   <h2 class="title editable-title" contenteditable="{{ $isEditable ? 'true' : 'false' }}" data-id="{{ $headingId }}" > {{ $headingTitle }}</h2>
    ...
    <p class="lead px-lg-10 px-xxl-9 editable-description" contenteditable="{{ $isEditable ? 'true' : 'false' }}" data-id="{{ $headingId }}" >{{ $headingDescription }}</p>
    ...
    <div class="pricing {{ $planItem->name === 'Silver' ? 'pricing-featured bg-primary' : 'pricing' }}  ">
        <div class="pricing-body p-5">
            ...
            <ul class="list gy-3 pt-4 ps-2">
                ...
                <li><em class="icon ni ni-check text-success"></em> <span>English Content Output</span></li>
